#!/bin/bash
#SBATCH --job-name=strange_gevp 
#SBATCH --account=exotichadrons 
#SBATCH --nodes=1       
#SBATCH --ntasks-per-node=1
#SBATCH --time=24:00:00             
#SBATCH --output=contract-log/a1m_strange%a.log        
...
CUDA_VISIBLE_DEVICES=0 srun -n 1 --gres=gpu:1 -c 16 python3 contract_2pt_matrix_gpu.py --ini $INI --cfg_ids $(echo ${CFG_IDS} | tr ' ' ',') --task_id ${SLURM_ARRAY_TASK_ID} --gpu --strange

#CUDA_VISIBLE_DEVICES=0 srun -n 1 --gres=gpu:1 -c 16    parser.add_argument('--gpu', action='store_true', help="Enable GPU acceleration")
#python3 contract_2pt_matrix_gpu.py --ini $INI --cfg_ids 11,12 --task_id ${SLURM_ARRAY_TASK_ID} --gpu --strange
