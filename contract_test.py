...
def correlator_matrix(operators, peram_dir, meson_dir, peram_strange_dir, nt, channel, cfg_id, ntsrc, nvec, strange):
            ...
            for snk_idx, (snk_name, snk_op) in enumerate(operators.items()):
                for tsrc in range(ntsrc):
                    for t in range(nt):
                        ...
                        print(f'Performing contraction for {src_name}-{snk_name}, tsrc {tsrc}, timeslice {t}',flush=True)
    ...
    print("HDF5 file successfully written with GEVP data.",flush=True)
...
def get_valid_cfgs(start_cfg, end_cfg, invalid_cfgs):
    """
...
def main(in_file, strange):
    ...
    try:
        print(f"Rank {rank} will now read/write file {cfg_id}...",flush=True)
        print(f"Rank {rank} has finished reading/writing file {cfg_id}.",flush=True)
        correlator_matrix(
            operators=operators,
            channel=channel,
            cfg_id=cfg_id,
...
if __name__ == '__main__':
    ...
    print("Reading arguments",flush=True)
