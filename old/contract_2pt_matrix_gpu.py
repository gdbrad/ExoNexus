...
def contract_local(meson_file, nt, nvec, operator, t, gpu=False):
    ...
    phi_0 = einsum("ij,ab->ijab", operator.gamma, D0[0], optimize="optimal")
...
def contract_nabla(meson_file, nt, nvec, operator, t, gpu=False):
    ...
    nabla_t = sum(
        einsum("ij,ab->ijab", operator.gamma @ gamma_i[i], D1[t] if i == 0 else D2[t] if i == 1 else D3[t], optimize="optimal")
        for i in range(3)
    )
...
def contract_B_D(meson_file,nt,nvec,operator, t, add=True,gpu=False):
    ...
    # src terms (t=0)
    D2D3_phi_0_1 = einsum("ij,ab->ijab", gamma_i[0], D2D3[0], optimize="optimal")
    D2D3_phi_0_2 = einsum("ij,ab->ijab", gamma_i[0], D3D2[0], optimize="optimal")  # Subtract this one

    D2D3_phi_0_3 = einsum("ij,ab->ijab", gamma_i[1], D3D1[0], optimize="optimal")
    D2D3_phi_0_4 = einsum("ij,ab->ijab", gamma_i[1], D1D3[0], optimize="optimal")  # Subtract this one

    D2D3_phi_0_5 = einsum("ij,ab->ijab", gamma_i[2], D1D2[0], optimize="optimal")
    D2D3_phi_0_6 = einsum("ij,ab->ijab", gamma_i[2], D2D1[0], optimize="optimal")  # Subtract this one
    ...
    # snk terms (t)
    D2D3_phi_t_1 = einsum("ij,ab->ijab", gamma_i[0], D2D3[t], optimize="optimal")
    D2D3_phi_t_2 = einsum("ij,ab->ijab", gamma_i[0], D3D2[t], optimize="optimal")  # Subtract this one

    D2D3_phi_t_3 = einsum("ij,ab->ijab", gamma_i[1], D3D1[t], optimize="optimal")
    D2D3_phi_t_4 = einsum("ij,ab->ijab", gamma_i[1], D1D3[t], optimize="optimal")  # Subtract this one

    D2D3_phi_t_5 = einsum("ij,ab->ijab", gamma_i[2], D1D2[t], optimize="optimal")
    D2D3_phi_t_6 = einsum("ij,ab->ijab", gamma_i[2], D2D1[t], optimize="optimal")  # Subtract this one
...
def correlator_matrix(task_id, use_pickle, operators, peram_dir, meson_dir, peram_strange_dir, nt, channel, cfg_id, ncfg, ntsrc, nvec, debug, strange,gpu=False):
    ...
    if strange:
        h5_file = f"h5-out/test_gevp_strange_{channel}_{timestr}_{task_id}.h5" 
    else:
        h5_file = f"h5-out/test_gevp_light_{channel}_{timestr}_{task_id}.h5" 
...
def main(in_file, cfg_ids, task_id, gpu,strange):
    ...
    print(f"Processing configurations: {cfg_ids}")
