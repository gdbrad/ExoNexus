...
myid_All = comm_All.Get_rank()
numprocs_All = comm_All.Get_size()

# valid_cfgs = [cfg for cfg in range(11, 2000, 10) if cfg not in {1001, 1991}]

# if myid_All < len(valid_cfgs):
#     cfg_id = valid_cfgs[myid_All]  
# else:
#     cfg_id = None  

# Print rank assignments for debugging
# print(f"Rank {myid_All} of {numprocs_All} assigned cfg_id {cfg_id}")
# 4 50 tasks single node ind. jobs 
cfg_id=(myid_All+1)*10 + 1
...
# Print rank assignments for debugging
print(f"Rank {myid_All} assigned cfg_id {cfg_id}")

print(myid_All,numprocs_All)
...
def correlator_matrix(operators, peram_dir, meson_dir, peram_strange_dir, nt, channel, cfg_id, ntsrc, nvec, strange):
    ...
    if strange:
        h5_file = f"h5-mpi/test_gevp_strange_{channel}_{timestr}_{cfg_id}.h5" 
    else:
